...
(function () {
  ...
  /** 風防(サファイア): ほぼフラットな板ガラス。中央に白い曇り・盛り上がりを作らない。
     ・大きなドームを廃止し、平らな薄い円板 + 薄い立ち上がり縁で構成。
     ・ほぼ無色透明。反射は控えめで、縁と傾いた面にだけ細く現れる(bloom・発光なし)。 */
  ...
  /** ベゼル: IWCポートフィノ寄りの細く控えめなリング(額縁)。
     ・幅を細く・高さを低く。断面は大きな山型にせず、浅く滑らかな傾斜。
     ・外周エッジに細いハイライトが入る程度で、鏡面反射を強くしすぎない。
     ・仕上げバリエーション(polished / gold / fluted)対応。 */
  function buildBezel(p) {
    const outer = p.outer || 30, inner = p.inner || 26.4, h = p.height || 0.85;
    // 浅く滑らかな傾斜: 内周から緩やかに立ち上がり、外周直前で頂点、細い外エッジへ落とす
    const pts = [
      new THREE.Vector2(inner, 0),
      new THREE.Vector2(inner, h * 0.62),
      new THREE.Vector2(inner + (outer - inner) * 0.55, h),
      new THREE.Vector2(outer - 0.35, h * 0.82),
      new THREE.Vector2(outer, h * 0.42),
